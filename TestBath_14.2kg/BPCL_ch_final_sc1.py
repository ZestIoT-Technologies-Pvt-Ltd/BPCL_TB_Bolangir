...
sc1=VideoStream('172.20.59.22',8096)
...
if __name__ == '__main__':
	...
	try:
		...
		while True:
			...
			moving,img1,track_dict,st_dict,count,cyl = XY_track.track(img1,darknet_image_T,network_T,class_names_T,track_dict,st_dict,count,cyl,moving)
			moving =True
			if moving == True:
				if idle_time != 0:
					...
					idle_time = 0
				input_image, draw_image, output_scale = posenet.read_imgfile(img1, scale_factor=0.71, output_stride=output_stride)
				...
				if number_roi == 0 :
					...
					if output == True :
						number_roi, number_view = 1,1
					else :
						number_roi, number_view = 0,0
				...
				if number_roi >= 1 and number_view == 0:
					...
					View_draw =  False
				if number_roi >= 1 and number_view >= 0 and number_motion == 1:
					Timer.timer("motion",True,cam1)
					Motion_draw = True
				if number_roi >= 1 and number_view >=  0 and number_motion == 0:
